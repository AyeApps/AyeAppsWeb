...
export default function InteractiveDots({ className = '' }: InteractiveDotsProps) {
  ...
  useEffect(() => {
    ...
    // Pointer coordinates (mouse or touch) relative to container
    const pointer = {
      x: -2000,
      y: -2000,
      active: false,
    ...
    const spacing = 28 // Distance between dots (matching 28px standard)
    const baseRadius = 1.3
    const maxRadius = 2.2 // Subtle, elegant expansion on hover/touch
    const hoverRadius = 145 // Reactive influence radius
    // Continuous drifting speed (px per millisecond) ~7.5px per second
    const driftSpeedX = 0.0075
    ...
    // Visibility observer to pause animation when scrolled off-screen
    ...
    // Mobile Touch Tracking (Passive to ensure 100% smooth scrolling)
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          if (currentIntensity > 0.02) {
            // Reactive illuminated state (Amber)
